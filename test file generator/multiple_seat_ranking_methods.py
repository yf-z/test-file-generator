...
from helpers import CompareMethodIfEqual, ElectionManager, ElectionResults
...
def preferential_block_voting(
    ...
    while True:
        ...
        candidates_in_race_votes: [int] = [manager.get_number_of_votes(candidate) for candidate in candidates_in_race]
...
def single_transferable_vote(
    ...
    votes_needed_to_win: int = math.floor(voters / (seats + 1))+1  # Droop quota
    ...
    while True:
        ...
        for i, candidate in enumerate(candidates_in_race):
            ...
            if (votes_for_candidate) >= votes_needed_to_win:
                candidates_to_elect.append(candidate)

            # Reject candidates that even with redistribution can't change the results
            elif i >= seats_left and (votes_remaining) <= last_votes:
        ...
        if manager.get_number_of_candidates_in_race() == 0:
            ...
            for candidate in candidates_to_elect:
                ...
                excess_votes: int = votes_for_candidate - votes_needed_to_win
            ...
            for candidate in candidates_to_reject:
                votes_for_candidate: int = manager.get_number_of_votes(candidate)
